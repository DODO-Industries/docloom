...
# Hot-reload is checked at most this often (wall-clock seconds), not on every
# single get() — physics/cognition code calls get_float()/get_int() tens of
# thousands of times per second at scale, and an os.stat() per call was ~13%
# of total ingest time for no behavioral benefit (a config edit is still
# picked up within one interval). See module_AI performance investigation.
RELOAD_CHECK_INTERVAL_S = 1.0
...
class TuningManager:
    ...
    def reload_if_changed(self):
        now = time.monotonic()
        if now - self._last_check_wall < RELOAD_CHECK_INTERVAL_S:
            return
        self._last_check_wall = now
