...
class AssemblyCompilation:
    ...
    def compile_assembly(self,
                         ...
                         actual_vec: Optional[np.ndarray] = None) -> Optional[CognitiveAssembly]:
        """
        ...
        # Mechanism 2 (surprise-gated consolidation): track a per-concept
        # expected vector (EMA of what this attractor has actually looked
        # like) and accumulate "surprise" — how far each new occurrence
        # lands from that expectation. A region under active contradiction
        # racks up surprise fast; a region of consistent facts barely moves
        # it. maybe_sleep_cycle reads mem["surprise_accum"] to wake early.
        if actual_vec is not None:
            if "expected_vec" not in mem:
                mem["expected_vec"] = safe_normalize(actual_vec.copy())
                mem["surprise_accum"] = 0.0
            else:
                sim = cosine_similarity_vec(mem["expected_vec"], actual_vec)
                surprise = 1.0 - max(float(sim), 0.0)
                mem["surprise_accum"] = mem.get("surprise_accum", 0.0) + surprise
                alpha = tuning_manager.get_float("EXPECTATION_EMA_ALPHA", 0.2)
                mem["expected_vec"] = safe_normalize(mem["expected_vec"] * (1 - alpha) + actual_vec * alpha)
