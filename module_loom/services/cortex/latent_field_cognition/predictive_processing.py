...
class CausalGraphs:
    ...
    def record_transitions_batch(self, pairs, weight: float = 1.0, prediction_error: float = 0.5):
        """Add many transitions at once, decaying stale edges only ONCE at the end.
        record_transition() calls decay_stale_edges() per edge — an O(E) scan each
        time — which is fine for a single ingest but O(E*N) for a batch. This is the
        batch path used by deferred recall-consolidation (Hebbian co-occurrence)."""
        causal_strength = weight * (1.0 - prediction_error)
        if causal_strength < tuning_manager.get_float("CAUSAL_THRESHOLD", 0.1):
            return
        now = time.time()
        for a, b in pairs:
            if a == b:
                continue
            if self.causal_matrix.has_edge(a, b):
                e = self.causal_matrix[a][b]
                e['weight'] = (e['weight'] + causal_strength) / 2
                e['last_seen'] = now
                e['frequency'] += 1
            else:
                self.causal_matrix.add_edge(a, b, weight=causal_strength, last_seen=now, frequency=1)
        self.decay_stale_edges()
