...
from module_loom.services.weaver.substrate_layout import LoomStore, detect_version, UNBUCKETED
...
def find_brain_dir(explicit: Optional[str] = None) -> str:
    """
    Resolves the .brain_data directory: an explicit path, then the configured
    BRAIN_STORAGE_DIR, then the two legacy fallback locations DocLoom has used.
    """
    if explicit:
        if not os.path.isdir(explicit):
            raise FileNotFoundError(f"Not a directory: {explicit}")
        return os.path.abspath(explicit)
    from module_loom.config.env_config import BRAIN_STORAGE_DIR
    if os.path.isabs(BRAIN_STORAGE_DIR) and os.path.isdir(BRAIN_STORAGE_DIR):
        return os.path.abspath(BRAIN_STORAGE_DIR)
    custom = os.path.join(PROJECT_ROOT, BRAIN_STORAGE_DIR)
    ...
    raise FileNotFoundError(
        "No brain data found at configured BRAIN_STORAGE_DIR, assets/.brain_data, or ./.brain_data. "
...
def _load_live_state(universe: Optional[UniverseContainer]) -> Tuple[Dict[str, Any], Dict[int, Tuple[float, int]], Dict[str, str]]:
    """
