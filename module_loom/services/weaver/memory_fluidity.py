...
class DynamicMemoryFluidity:
    ...
    def calculate_decay(
        ...
        # Continuum Equation with Non-Zero Resting Baseline Potential:
        # A(t) = A_base + (A0 - A_base) * e^(-λ_eff * Δt)
        base_resting = tuning_manager.get_float("BASE_RESTING_ENERGY", 0.15)
        excess = max(0.0, initial_activation - base_resting)
        decayed_activation = base_resting + excess * math.exp(-lambda_effective * elapsed_time)
    ...
    def calculate_decay_batch(
        ...
        excess = np.maximum(0.0, initial_activations - base_resting)
        return base_resting + excess * np.exp(-lambda_effective * elapsed)
